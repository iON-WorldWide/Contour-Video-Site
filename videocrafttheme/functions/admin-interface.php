...
function inkthemes_load_only() {
	...
	wp_register_script('jquery-tiny_mce', ADMINURL .'/js/tiny_mce/tiny_mce.js', array( 'jquery' ));
	...
	function of_admin_head() { 
		...
		echo '<link rel="stylesheet" type="text/css" href="'. get_template_directory_uri().'/functions/admin-style.css" media="screen" />';
		 ...
		 // COLOR Picker ?>
<script type="text/javascript" src="<?php echo ADMINURL; ?>js/tiny_mce_init.js"></script>
	...
	foreach ($options as $value) {
		...
		case 'textarea':
			...
			if(isset($value['std'])) {
				...
				if( $std != "") { $ta_value = stripslashes( $std ); }
$class = ''; if(isset( $value['class'] )) { $class = $value['class']; } else { $class = 'of-input';}
